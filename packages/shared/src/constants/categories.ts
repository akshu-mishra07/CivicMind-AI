...
export const CATEGORY_MAP = new Map(
  ISSUE_CATEGORIES.map((c) => [c.id, c])
);

export function getCategoryConfig(id: IssueCategory): CategoryConfig | undefined {
...
export function getCategoryDepartment(id: IssueCategory): string {
  return CATEGORY_MAP.get(id)?.department ?? 'GENERAL';
}
